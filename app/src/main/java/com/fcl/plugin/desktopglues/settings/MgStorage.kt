...
interface MgStorage {
    ...
    /**
     * 删除 DesktopGlues 和本插件在 MG 目录下自己创建的全部文件（危险区域的「移除 DesktopGlues」）。
     *
...
/**
 * DesktopGlues（native 库）和本插件会在 MG 目录下主动创建的全部文件名。
 *
 ...
 * 先写临时文件再 rename。
 *
 * 同目录下的 rename 是原子的，所以读的一方（游戏里的 DesktopGlues）要么看到旧内容，
